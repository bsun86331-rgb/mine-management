...
document.addEventListener("DOMContentLoaded", function () {
    ...
    */
    ...
    function bindEvents() {
        ...
        $("leaveType")
            ?.addEventListener(
                "change",
                refreshLeaveApplicationPreview
            );
    ...
    */
    ...
    function openLeaveModal() {
        ...
        initializeHalfDayLeaveSelection();

        renderRotationLeavePolicy();
    ...
    */
    ...
    function getCurrentPersonnelRecord() {
    ...
    */
    ...
    function getRotationLeaveCycleInfo(
        person,
        referenceDate
    ) {

        const enabled =
            person?.rotationLeaveEnabled ===
                true ||
            person?.rotationLeaveEnabled ===
                "true";
        ...
        const cycleMonths =
            Number(
                person?.rotationLeaveCycleMonths ||
                0
            );
        ...
        let anchor =
            parseLocalDateKey(
                person.rotationLeaveCycleAnchor
            );
    ...
    */
    ...
    function getRotationLeaveUsage(
        person,
        cycle
    ) {
        ...
        getMyLeaveRequests()
            .forEach(
                item => {
                    ...
                    const sameCycleBySnapshot =
                        Boolean(
                            item.rotationCycleStart &&
                            item.rotationCycleStart ===
                                cycle.startKey
                        );
                        ...
                        Math.max(
                            ...
                            Number(
                                item.status ===
                                    "returned"
                                    ? (
                                        item.actualRotationCoveredDays ??
                                        0
                                      )
                                    : (
                                        item.rotationCoveredDays ??
                                        item.leaveDays ??
                                        0
                                      )
                            ) ||
    ...
    function renderRotationLeavePolicy(
        referenceDate
    ) {

        const person =
            getCurrentPersonnelRecord();


        const cycle =
            getRotationLeaveCycleInfo(
                person,
                referenceDate
            );
    ...
    */
    ...
    function initializeHalfDayLeaveSelection() {
        ...
        syncHalfDayLeaveFields();
    }


    function buildHalfDayBoundary(
        dateKey,
        half,
        isEndBoundary
    ) {
    ...
    function syncHalfDayLeaveFields() {
    ...
    function getRequestedLeaveDays() {
    ...
    function getLeaveAttendanceImpact(
        type,
        days,
        usage
    ) {
    ...
    function refreshLeaveApplicationPreview() {
        ...
        if (
            ...
            impactBox.innerHTML =
                "请选择正确的开始日期、结束日期和上午 / 下午。结束时段必须晚于开始时段。";
        ...
        if (
            ...
            impactBox.innerHTML =
                ...
                " 天；审批通过后扣考勤 " +
                formatLeaveDays(
                    impact.attendanceDeductionDays
                ) +
                " 天。";
    ...
    function submitLeave() {
        ...
        records.push({
            ...
            rotationRemainingBeforeRequest:
                cycle.enabled
                    ? usage.remainingDays
                    : 0,
                ...
                item => {
                    ...
                    return `
                        <div class="record-card">
                            ...
                            ${
                                Number(item.leaveDays || 0) > 0
                                    ? `
                                        <div class="record-note">
                                            申请：
                                            ${escapeHtml(formatLeaveDays(item.leaveDays))}
                                            天
                                            ${
                                                item.leaveType === "轮休假"
                                                    ? ` · 轮休额度内 ${escapeHtml(formatLeaveDays(item.rotationCoveredDays || 0))} 天 · 扣考勤 ${escapeHtml(formatLeaveDays(item.attendanceDeductionDays || 0))} 天`
                                                    : ` · 扣考勤 ${escapeHtml(formatLeaveDays(item.attendanceDeductionDays ?? item.leaveDays ?? 0))} 天`
                                            }
                                        </div>
                                    `
                                    : ""
                                ...
                                item.status === "returned"
                                    ? `
                                        <div class="record-note">
                                            已销假：
                                            实际请假
                                            ${escapeHtml(formatLeaveDays(item.actualLeaveDays || 0))}
                                            天
                                            · 实际扣考勤
                                            ${escapeHtml(formatLeaveDays(item.actualAttendanceDeductionDays || 0))}
                                            天
                                            ${
                                                item.leaveType === "轮休假"
                                                    ? ` · 实际轮休额度占用 ${escapeHtml(formatLeaveDays(item.actualRotationCoveredDays || 0))} 天`
                                                    : ""
                                            }
                                            <br>
                                            销假时间：
                                            ${escapeHtml(formatDateTime(item.returnedAt))}
                                        </div>
                                    `
                                    : ""
                            }
                            ...
                            ${
                                canReturnFromApprovedLeave(item)
                                    ? `
                                        <button
                                            type="button"
                                            class="mini-danger-button"
                                            data-return-leave="${escapeHtml(item.leaveId)}"
                                        >
                                            ${
        ...
        .forEach(
            button => {
                ...
                button.addEventListener(
                    "click",
                    function () {
                        returnFromApprovedLeave(
                            button.dataset.returnLeave
                        );
                    }
                );
            }
    ...
    规则：
    - 只有 status=approved 且原结束时间尚未到达时可以销假。
    - 请假尚未开始时，可取消已批准请假，实际使用0天。
    - 已经开始后提前返岗，实际天数按0.5天为最小单位向上取整。
    - 销假后 status=returned，立即退出“批准请假”状态，
      调度端可重新把该司机列为可调度人员。
    - 不自动恢复原生产任务，仍由调度重新安排。
    */

    function canReturnFromApprovedLeave(
        item
    ) {
    ...
    function roundLeaveUsageToHalfDay(
        start,
        returnedAt,
        maximumDays
    ) {
    ...
    function buildEarlyReturnImpact(
        item,
        actualLeaveDays
    ) {
        ...
        if (
            ...
            const actualRotationCoveredDays =
                Math.min(
                    actualLeaveDays,
                    originalCovered
                );
            ...
            return {
                actualRotationCoveredDays,

                actualAttendanceDeductionDays:
                    Math.max(
                        0,
                        actualLeaveDays -
                        actualRotationCoveredDays
    ...
    function returnFromApprovedLeave(
        leaveId
    ) {
        ...
        if (
            !canReturnFromApprovedLeave(
                item
            )
        ) {

            alert(
                "这条请假当前不能销假，可能已经结束或状态已发生变化。"
            );
        ...
        const message =
            beforeStart
                ...
                : (
                    "确认提前销假吗？\n\n" +
                    "系统会按0.5天为最小单位计算实际请假天数，" +
                    "销假后你会立即恢复为可调度状态，" +
                    "但原生产任务不会自动恢复，需要调度重新安排。"
                  );
        ...
        const impact =
            buildEarlyReturnImpact(
                item,
                actualLeaveDays
            );


        item.originalApprovedEndTime =
            item.originalApprovedEndTime ||
            item.endTime ||
            "";
        ...
        item.returnedBeforeStart =
            beforeStart;


        item.actualLeaveDays =
            actualLeaveDays;


        item.actualRotationCoveredDays =
            impact.actualRotationCoveredDays;


        item.actualAttendanceDeductionDays =
            impact.actualAttendanceDeductionDays;


        item.actualRotationExcessDays =
            item.leaveType ===
                "轮休假"
                ? impact.actualAttendanceDeductionDays
                : 0;
